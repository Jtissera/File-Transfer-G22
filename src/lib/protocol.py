...
# Agrego esos padding para alinear el struct,
# mejorando la eficiencia y la facil lectura.
...
# Flujo de carga
#
# El cliente envía UPLOAD_REQ con el nombre de archivo + tamaño de archivo
# en el payload.

# El servidor responde con ACK(0) con el puerto de transferencia para
...
# El servidor sabe que la transferencia se ha completado cuando los
# bytes recibidos == tamaño del archivo.
# ---------------------------------------------------------------------------
...
def parse_upload_req(payload: bytes) -> tuple:
    ...
    protocol = struct.unpack("!B", payload[offset + 4 : offset + 5])[0]

    return filename, filesize, protocol
...
# El cliente sabe que la transferencia se ha completado cuando los bytes
# recibidos == tamaño del archivo.
# ---------------------------------------------------------------------------


def build_download_req(filename: str, protocol: int) -> bytes:
    """
...
def parse_download_req(payload: bytes) -> tuple:
    ...
    name_len = struct.unpack("!H", payload[:2])[0]
    filename = payload[2 : 2 + name_len].decode("utf-8")
    protocol = struct.unpack("!B", payload[2 + name_len : 2 + name_len + 1])[0]
...
def build_ack_with_port(ack: int, port: int) -> bytes:
    """ACK con puerto de transferencia (utilizado en upload)."""
...
def build_ack_with_port_and_size(ack: int, port: int, filesize: int) -> bytes:
    """
    ACK con puerto de transferencia y
    tamaño de archivo (utilizado en download).
    """
